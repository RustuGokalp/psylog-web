...
export default function PostForm({
  ...
  const formik = useFormik<PostFormValues>({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      const published = values.publishMode === "publish";
      const publishAt =
      ...
      try {
        if (mode === "create") {
          ...
          if (!postId) throw new Error("postId is required for edit mode");
  ...
  // Parse publishAt string ("yyyy-MM-dd'T'HH:mm:ss") → Date for calendar
  ...
  function handleCalendarSelect(day: Date | undefined) {
    ...
    formik.setFieldValue(
      "publishAt",
      format(day, "yyyy-MM-dd'T'HH:mm:ss"),
    );
  ...
  function handleTimeChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    formik.setFieldValue(
      "publishAt",
      format(updated, "yyyy-MM-dd'T'HH:mm:ss"),
    );
  }
      ...
      <div className="flex flex-col gap-3">
        ...
        {formik.values.publishMode === "schedule" && (
          <div className="flex flex-col gap-2 rounded-lg border border-violet-100 bg-violet-50 p-4">
            ...
            {formik.touched.publishAt && formik.errors.publishAt && (
              <p className="text-xs text-red-500">{formik.errors.publishAt}</p>
            )}
